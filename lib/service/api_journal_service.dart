...
class ApiJournalService {
  ...
  Future<List<Map<String, dynamic>>> getJournalEntries() async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/logs'), headers: await _getAuthHeaders())
  ...
  Future<Map<String, dynamic>> createJournalEntry(
    ...
    try {
      ...
      print('🔵 Sending to API: ${jsonEncode(entry)}'); // Debug

      final response = await http
          .post(
            Uri.parse('$baseUrl/logs'), // Changez le endpoint
            headers: fullHeaders,
            body: jsonEncode(
              entry,
            ), // Envoyez les données directement sans encapsuler
          )
          .timeout(const Duration(seconds: 10));
