...
class NotificationController extends GetxController {
  ...
  final UserController _userController =
      Get.find<UserController>(); // Accès au UserController
  ...
  Future<void> loadNotifications() async {
    ...
    try {
      ...
      final userNotifications =
          allNotifications
              .where((notif) => notif.receiverId == currentUser.id)
              .toList();
      ...
      unreadCount.value =
          userNotifications.where((notif) => !notif.isRead).length;
    } catch (e) {
  ...
  // Ajoutez cette méthode spécifique aux commandes
  void handleNewOrder(Map<String, dynamic> orderData) {
    final notification = NotificationModel(
      id: DateTime.now().millisecondsSinceEpoch, // ID temporaire
      senderId: orderData['buyer_id'] ?? 0,
      receiverId: orderData['seller_id'] ?? 0,
      message: 'Nouvelle commande #${orderData['id']} - ${orderData['total']}€',
      type: 'new_order',
      isRead: false,
      createdAt: DateTime.now(),
      // Stockez les données supplémentaires dans conversationId (réutilisez le champ)
      conversationId: orderData['id'],
    );

    addNotification(notification);

    // Option: Notification locale
    Get.snackbar(
      'Nouvelle commande',
